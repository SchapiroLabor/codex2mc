...
def write_markers_file(data_path, ref_marker="DAPI"):
    ...
    for img in img_paths:
        img_name = [img.stem]
        cycle_no = extract_values(
            patt["cycle_number"][0], img_name, number_cast=patt["cycle_number"][1]
        )
        background = extract_values(
            patt["background"][0], img_name, number_cast=patt["background"][1]
        )
        markers = extract_values(
            patt["marker_name"][0], img_name, number_cast=patt["marker_name"][1]
        )[0].split("__")
        filters = extract_values(
            patt["Filter"][0], img_name, number_cast=patt["Filter"][1]
        )[0].split("__")
        ome = from_tiff(img)
        ...
        if background[0] == "B":
            ...
            markers = [
                "bg_{c}_{m}-{f}".format(c=f"{cycle_no[0]:03d}", m=x, f=y)
        ...
        mks_file["exposure"].extend(
            [
                ome.images[0].pixels.planes[ch].exposure_time
                for ch, _ in enumerate(markers)
            ]
        )
